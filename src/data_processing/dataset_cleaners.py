...
from src.utils import SpellCorrectionModel
...
class StopWordsDatasetCleaner(DatasetCleaner):
    ...
    def clean_text(self, text: str) -> str:
        cleaned_text = [
            word for word in word_tokenize(text) if word not in self.stopwords
        ]
...
class PunctuationDatasetCleaner(DatasetCleaner):
    ...
    def clean_words(self, words: list[str]) -> list[str]:
        return [
            word.translate(self.table) for word in words if word.translate(self.table)
        ]
...
class DatasetCleanderManager:
    def __init__(self, dataset_cleaners: dict[str, DatasetCleaner]):
